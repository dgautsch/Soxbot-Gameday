...
class Schedule(object):
    ...
    def __init__(self):
        ...
        self.API_URL = 'http://api.sportradar.us/mlb-t5/games/' + \
            str(self.YEAR) + '/REG/schedule.xml?api_key=' + self.config.API_KEY
    ...
    def getSchedule(self):
        ...
        for game in self.ROOT.iter(self.XML_NS + 'game'):
            ...
            newdate = dateutil.parser.parse(
                game.get('scheduled'), tzinfos=self.TZ)
    ...
    def createTable(self):
        ...
        for i in range(len(schedule)):
            table.append('|' + schedule[i]['home'] + '|' + schedule[i]['away'] +
                         '|' + schedule[i]['gametime'] + '|' + schedule[i]['network'] + '|')
    ...
    def postSchedule(self):
        ...
        new_description = re.sub(
            '(\[\/\/\]: # \((Begin Schedule)\))([^"]*)(\[\/\/\]: # \((End Schedule)\))', new_schedule, description)
